...
google.maps.Map = function(mapDiv, opts) {
	...
	//required
	options.center = opts.center.coordinate;
...
google.maps.Map.prototype._domEventsListener = function(event) {
	google.maps.event.trigger(this, event.type, this._createCustomDomEvent(event));
};

google.maps.Map.prototype._createCustomDomEvent = function(event) {
	var customEvent = {
			event: event,
			stop: function() {event.stopPropagation()},
		    latLng: google.maps.LatLng.fromCoordinate(event.display.pixelToGeo(event.displayX, event.displayY)),
